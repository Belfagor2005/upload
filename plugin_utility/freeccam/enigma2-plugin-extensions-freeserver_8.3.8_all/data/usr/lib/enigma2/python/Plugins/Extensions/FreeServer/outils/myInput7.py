...
BFNAME= 'userbouquet.IPTVWORLD23.tv'
LastScanned= 'userbouquet.LastScanned.tv'
M3UPATH= '/tmp/IPTVWORLD23.m3u'
m3ufile= 'IPTVWORLD23'
WGET='wget --no-check-certificate'
BOUQUETSPATH='/etc/enigma2/bouquets.tv'
E2SETTINGSPATH='/etc/enigma2'
url2="https://iptv-up.com/client/clientarea.php?action=productdetails"
url22='https://iptv-up.com/client/includes/verifyimage.php'
services_url='https://iptv-up.com/client/clientarea.php?action=services'
################################################################################
...
Post_Hdr={'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/64.0.3282.140 Safari/537.36 Edge/18.17763',
          ...
          'Referer':'https://iptv-up.com/client/cart.php',
...
<widget name="HelpWindow" position="200,370" size="1,1" alphatest="on" foregroundColor="#00ffffff" transparent="1" zPosition="6" />
    ...
    def __init__(self, session, useableChars=None, **kwargs):
        ...
        if dwidth == 1280:
            self.skin = myInput7.skinhd
        else:
            self.skin = myInput7.skinfhd
...
### End
        ...
        post_data2='customfield[114]=M3U&configure=true&i=0&submit=Continue'                             
        ...
        post_account=s.post('https://iptv-up.com/client/cart.php?a=add&pid=25',headers=Post_Hdr,data=post_data2,verify=False,allow_redirects=True).content
        post_account=s.post('https://iptv-up.com/client/cart.php?a=confproduct&i=0',headers=Post_Hdr,data=post_data2,verify=False,allow_redirects=True).content
        data=s.get('https://iptv-up.com/client/cart.php?a=view',verify=False,allow_redirects=True).text
        regx='token" value="(.*?)"'
    ...
    def save(self):
        ...
        if pathExists(Distnt):
            ...
            if Password != '':
                ...
                self.session.open(Console3, title='IPTVWORLD23', cmdlist=cmdlist, finishedCallback=None)
                self.close(True)
    ...
    def askForWord(self):
             ...
             if os.path.exists(LINKFILE2):
                ...
                SRV_HDR={'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; rv:86.0) Gecko/20100101 Firefox/86.0',
                         ...
                         'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8',
                         'Accept-Language': 'fr,fr-FR;q=0.8,en-US;q=0.5,en;q=0.3',
                         'Referer': 'https://iptv-up.com/client/clientarea.php?action=productdetails&id=%s'%service_id,
                         'Content-Type':'application/x-www-form-urlencoded; text/html; charset=utf-8',
                         'DNT': '1',
                         'Upgrade-Insecure-Requests': '1',
                         'Accept-Encoding': 'gzip'}#gzip, deflate, br'
                #post_data=s.post('https://ssdvpshosting.xyz/store/clientarea.php?action=productdetails&id=%s'%service_id,headers=SRV_HDR,data=params,verify=False,allow_redirects=True).text
                ...
                if "M3U" in data:
                    ...
                    regx2 = '''username" value="(.*?)"'''
                    regx3 = '''password" value="(.*?)"'''
                    LINE = "/tmp/link2 APK Androit Box TV: Smarters PRO .apk download >>>>>> https://tinyurl.com/smartersap"+'\n'+'Username :'+re.findall(regx2, post_data, re.M|re.I)[0]+'\n'+"Password :"+re.findall(regx3, post_data, re.M|re.I)[0]+'\n'+"Host : http://ky-iptv.com:25461 URLs http://ky-tv.cc:25461 or kytv.xyz:25461"
                    link="'"+re.findall(regx, post_data)[0]+"m3u?output=ts'"
                    if link:
                        print(("link :", link))
                        print(("LINE :", LINE))
