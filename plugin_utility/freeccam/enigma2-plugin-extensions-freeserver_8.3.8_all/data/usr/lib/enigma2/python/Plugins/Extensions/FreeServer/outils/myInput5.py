...
BFNAME= 'userbouquet.IPTVWORLD36.tv'
LastScanned= 'userbouquet.LastScanned.tv'
M3UPATH= '/tmp/IPTVWORLD36.m3u'
m3ufile= 'IPTVWORLD36'
WGET='wget --no-check-certificate'
BOUQUETSPATH='/etc/enigma2/bouquets.tv'
E2SETTINGSPATH='/etc/enigma2'
url2="https://iptvnow.uk/clientarea.php?action=productdetails"
url22='https://iptvnow.uk/includes/verifyimage.php'
services_url='https://iptvnow.uk/clientarea.php?action=services'
################################################################################
...
Post_Hdr={'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/64.0.3282.140 Safari/537.36 Edge/18.17763',
          ...
          'Referer':'https://client.torexiptv.com/cart.php',
...
<widget name="HelpWindow" position="200,370" size="1,1" alphatest="on" foregroundColor="#00ffffff" transparent="1" zPosition="6" />
    ...
    def __init__(self, session, useableChars=None, **kwargs):
        ...
        if dwidth == 1280:
            self.skin = myInput5.skinhd
        else:
            self.skin = myInput5.skinfhd
...
### End
        ...
        #post_data2='configure=true&i=0&submit=Continue'                             
        post_data2='customfield[1]='+EMAIL_TARGET+'&configure=true&i=0&submit=Continue'                            
        post_data='firstname='+NAME_TARGET+'&lastname='+LAST_TARGET+'&email='+EMAIL_TARGET+'&country-calling-code-phonenumber=1&phonenumber='+PHON_TARGET+'&address1='+ADRS_TARGET+'&address2='+ADRS_TARGET+'&city='+CITY_TARGET+'&state='+STATE_TARGET+'&postcode='+ZIP_TARGET+'&country=US&password='+RND_PASS+'&password2='+RND_PASS+'&applycredit=1&cccvv=&paymentmethod=paypal&ccinfo=new&ccnumber=5249092649854780&ccexpirydate=0123&cccvv=655&ccdescription=&notes=&marketingoptin=1&submit=Complete Order'
        post_account=s.post('https://iptvnow.uk/index.php/store/trials/free-trial-24-hours',headers=Post_Hdr,data=post_data2,verify=False,allow_redirects=True).text
        post_account=s.post('https://iptvnow.uk/cart.php?a=confproduct&i=1',headers=Post_Hdr,data=post_data2,verify=False,allow_redirects=True).text
        #data=s.get('https://iptvnow.uk/cart.php?a=viewt',verify=False,allow_redirects=True).text
        regx='token" value="(.*?)"'
        try:post_token=re.findall(regx, post_account, re.M|re.I)[0]
        except:pass                                                                                
    ...
    def save(self):
        ...
        if pathExists(Distnt):
            ...
            if Password != '':
                ...
                self.session.open(Console3, title='IPTVWORLD36', cmdlist=cmdlist, finishedCallback=None)
                self.close(True)
    ...
    def askForWord(self):
             ...
             if os.path.exists(LINKFILE2):
                ...
                #s.headers.update({'token': post_token})   
                checkout_data='token='+post_token+'&submit=true&custtype=new&loginemail=&loginpassword=&firstname='+NAME_TARGET+'&lastname='+LAST_TARGET+'&email='+NAME_TARGET+rnd2+'@gmail.com&country-calling-code-phonenumber=1&phonenumber='+RND_PASS+'&companyname=&address1='+ADRS_TARGET+'&address2='+ADRS2_TARGET+'&city='+CITY_TARGET+'&state='+STATE_TARGET+'&postcode='+ZIP_TARGET+'&country=US&password='+RND_PASS+'&password2='+RND_PASS+'&applycredit=1&paymentmethod=banktransfer&ccinfo=new&ccnumber=&ccexpirydate=&cccvv=&ccdescription=&marketingoptin=1&code='+code+''
                post_account=s.post('https://iptvnow.uk/cart.php?a=checkout',headers=Post_Hdr,data=checkout_data,verify=False,allow_redirects=True).text
                data=s.get(services_url,verify=False).text
                #regx='data-element-id="(.*?)"'
                regx='id=(.*?)">View Details</a></li>'
                try:service_id=re.findall(regx,data, re.M|re.I)[0]
                except:pass
                print("Service_id:",service_id)
                data=s.get('https://iptvnow.uk/clientarea.php?action=productdetails&id=%s'%service_id,allow_redirects=True,verify=False).text
