...
class OpenPanel(Screen):
    ...
    if HD_Res:
        ...
        <screen name="OpenPanel" position="240,100" size="800,540" title="OpenPanel">
            <widget name="list" position="10,20" size="780,520" scrollbarMode="showOnDemand" zPosition="1"/>
        </screen>"""
        
    else:
        skin = """
        <screen name="OpenPanel" position="160,70" size="400,460" title="OpenPanel">
            <widget name="list" position="5,5" size="390,450" scrollbarMode="showOnDemand" zPosition="1"/>
        </screen>"""
    ...
    def updateTitle(self):
        ...
        for attrName in attr.keys():
            if attrName == 'name':
                ...
                self.setTitle(str(self.newtitle))
                self["title"].setText(str(self.newtitle))
        ...
        if entry[1] == "entry":
            ...
            if tmp_xml is not None:
                ...
                try:
                    ...
                    self["summary_list"] = StaticText()
                    self.updateSummary()
                    if os.path.isfile("/tmp/.openpanel.xml") == True:
    ...
    def cancel(self):
        ...
        if backZap is not None or backZap > -1:
