...
sleep 0
cp /tmp/server /etc/tuxbox/config/supcam-emu  >/dev/null 2>&1 </dev/null &
sleep 0
cp /tmp/server /etc/tuxbox/config/supcam-emu/oscam.server  >/dev/null 2>&1 </dev/null &
sleep 0

find / -name "oscam.server" | xargs -I {} cp /tmp/server {}  >/dev/null 2>&1 </dev/null &
rm -rf /tmp/server >/dev/null 2>&1 </dev/null &
