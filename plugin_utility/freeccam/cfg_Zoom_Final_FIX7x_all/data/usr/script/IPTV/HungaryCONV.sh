...
sed -i "s/%/userbouquet.Hungary.m3u.tv/" /etc/enigma2/bouquets.tv 

echo "Stahování Hungary.m3u"
echo "Downloading Hungary.m3u"
curl  -k -Lbk -A -k -m 8 -m 52 -s  https://gitee.com/jjm2473/iptv/blob/master/channels/hu.m3u	 > /tmp/test/x
...
cat /tmp/test/x2 > /IPTV/World/Hungary.m3u
#########################################
cd /
rm -rf /tmp/test
echo ""
echo "converter Hungary.m3u for Enigma"
sleep 1
echo "start"
[ -d /tmp/test ] || mkdir -p /tmp/test
cp /IPTV/World/Hungary.m3u /tmp/test/Ru.m3u
...
echo "#NAME Hungary.m3u" > /etc/enigma2/userbouquet.Hungary.m3u.tv
cat /tmp/test/Rus10.m3u >> /etc/enigma2/userbouquet.Hungary.m3u.tv
rm -rf /tmp/test
cd /
