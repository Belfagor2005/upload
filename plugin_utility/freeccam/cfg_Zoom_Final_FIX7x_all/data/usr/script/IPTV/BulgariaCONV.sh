...
sed -i "s/%/userbouquet.Bulgaria.m3u.tv/" /etc/enigma2/bouquets.tv 

echo "Stahování Bulgaria.m3u"
echo "Downloading Bulgaria.m3u"
curl  -k -Lbk -A -k -m 8 -m 52 -s  https://gitee.com/jjm2473/iptv/blob/master/channels/bg.m3u	 > /tmp/test/x
...
cat /tmp/test/x2 > /IPTV/World/Bulgaria.m3u
#########################################
cd /
rm -rf /tmp/test
echo ""
echo "converter Bulgaria.m3u for Enigma"
sleep 1
echo "start"
[ -d /tmp/test ] || mkdir -p /tmp/test
cp /IPTV/World/Bulgaria.m3u /tmp/test/Ru.m3u
...
echo "#NAME Bulgaria.m3u" > /etc/enigma2/userbouquet.Bulgaria.m3u.tv
cat /tmp/test/Rus10.m3u >> /etc/enigma2/userbouquet.Bulgaria.m3u.tv
rm -rf /tmp/test
cd /
