...
sed -i "s/%/userbouquet.Ethiopia.m3u.tv/" /etc/enigma2/bouquets.tv 

echo "Stahování Ethiopia.m3u"
echo "Downloading Ethiopia.m3u"
curl  -k -Lbk -A -k -m 8 -m 52 -s  https://gitee.com/jjm2473/iptv/blob/master/channels/et.m3u	 > /tmp/test/x
...
cat /tmp/test/x2 > /IPTV/World/Ethiopia.m3u
#########################################
cd /
rm -rf /tmp/test
echo ""
echo "converter Ethiopia.m3u for Enigma"
sleep 1
echo "start"
[ -d /tmp/test ] || mkdir -p /tmp/test
cp /IPTV/World/Ethiopia.m3u /tmp/test/Ru.m3u
...
echo "#NAME Ethiopia.m3u" > /etc/enigma2/userbouquet.Ethiopia.m3u.tv
cat /tmp/test/Rus10.m3u >> /etc/enigma2/userbouquet.Ethiopia.m3u.tv
rm -rf /tmp/test
cd /
