...
sed -i "s/%/userbouquet.Canada.m3u.tv/" /etc/enigma2/bouquets.tv 

echo "Stahování Canada.m3u"
echo "Downloading Canada.m3u"
curl  -k -Lbk -A -k -m 8 -m 52 -s  https://gitee.com/jjm2473/iptv/blob/master/channels/ca.m3u	 > /tmp/test/x
...
cat /tmp/test/x2 > /IPTV/World/Canada.m3u
#########################################
cd /
rm -rf /tmp/test
echo ""
echo "converter Canada.m3u for Enigma"
sleep 1
echo "start"
[ -d /tmp/test ] || mkdir -p /tmp/test
cp /IPTV/World/Canada.m3u /tmp/test/Ru.m3u
...
echo "#NAME Canada.m3u" > /etc/enigma2/userbouquet.Canada.m3u.tv
cat /tmp/test/Rus10.m3u >> /etc/enigma2/userbouquet.Canada.m3u.tv
rm -rf /tmp/test
cd /
