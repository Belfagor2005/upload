...
sed -i "s/%/userbouquet.CostaRica.m3u.tv/" /etc/enigma2/bouquets.tv 

echo "Stahování CostaRica.m3u"
echo "Downloading CostaRica.m3u"
curl  -k -Lbk -A -k -m 8 -m 52 -s  https://gitee.com/jjm2473/iptv/blob/master/channels/cr.m3u	 > /tmp/test/x
...
cat /tmp/test/x2 > /IPTV/World/CostaRica.m3u
#########################################
cd /
rm -rf /tmp/test
echo ""
echo "converter CostaRica.m3u for Enigma"
sleep 1
echo "start"
[ -d /tmp/test ] || mkdir -p /tmp/test
cp /IPTV/World/CostaRica.m3u /tmp/test/Ru.m3u
...
echo "#NAME CostaRica.m3u" > /etc/enigma2/userbouquet.CostaRica.m3u.tv
cat /tmp/test/Rus10.m3u >> /etc/enigma2/userbouquet.CostaRica.m3u.tv
rm -rf /tmp/test
cd /
