...
sed -i "s/%/userbouquet.Slovakia.m3u.tv/" /etc/enigma2/bouquets.tv 

echo "Stahování Slovakia.m3u"
echo "Downloading Slovakia.m3u"
curl  -k -Lbk -A -k -m 8 -m 52 -s  https://gitee.com/jjm2473/iptv/blob/master/channels/sk.m3u	 > /tmp/test/x
...
cat /tmp/test/x2 > /IPTV/World/Slovakia.m3u
#########################################
cd /
rm -rf /tmp/test
echo ""
echo "converter Slovakia.m3u for Enigma"
sleep 1
echo "start"
[ -d /tmp/test ] || mkdir -p /tmp/test
cp /IPTV/World/Slovakia.m3u /tmp/test/Ru.m3u
...
echo "#NAME Slovakia.m3u" > /etc/enigma2/userbouquet.Slovakia.m3u.tv
cat /tmp/test/Rus10.m3u >> /etc/enigma2/userbouquet.Slovakia.m3u.tv
rm -rf /tmp/test
cd /
